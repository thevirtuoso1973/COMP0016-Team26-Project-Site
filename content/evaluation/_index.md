...
Our user interface implements the design principles of _Visibility, Feedback and Affordance_. The clear symbols or few words of explanation inside each button and the obviously interactive features reflect this. Affordance is implemented by how NudgeMe provides methods of keeping track of one’s wellbeing and sharing it, but these can be used flexibly by the user in whichever way suits them best. Our testers praised the UI for its simplicity. The wellbeing score circle and the step goal progress animation are two UI elements that we created from scratch, making the NudgeMe UI unique and memorable.

If we had time, an unread symbol to indicate an unseen step goal also would have been an excellent addition. Additionally, using a URL shortener for the deep link would prevent any confusion caused by the long and unfamiliar link.

Since our application’s target group has a broad range of ages, including older people who may not be as familiar with technology, the UI needed to be simple, straightforward and easy to use, and I believe we achieved this.
